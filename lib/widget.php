...
class LeagueManagerWidget extends WP_Widget
{
  ...
  /**
   * show next match box - old version
   *
   * @param int $number
   * @param array $instance
   ...
   * @return void
   */
  function showNextMatchBoxOld($number, $instance, $echo = true)
  {
  ...
  /**
   * show next match box vs - enhance viewport
   * TODO XXX Replace above function
   *
   * @param int $number
   * @param array $instance
   * @param boolean $echo (optional)
  ...
  function showNextMatchBox($number, $instance, $echo = true)
  {
    ...
    if ( $matches ) {
     ...
     // if ( !isset($match->title) ) $match->title = sprintf("%s &#8211; %s", $home_team, $away_team);
      ...
      //$out .= "<p class='match_title'><strong>". $match->title."</strong></p>";
      //$out .= "<p class='logos'><img class='away_logo' src='".$teams[$match->away_team]['logo']."' alt='' /></p>";
      ...
      // TODO XXX This needs to be dynamic
