...
class LeagueManager
{
  ...
  * retrieve match day
   ...
   * Duplicate of above- who knows if it will work
  ...
  function getLocation( $location_id )
  {
    ...
    $location->name = htmlspecialchars(stripslashes($location->name), ENT_QUOTES);
    $location->address = htmlspecialchars(stripslashes($location->address), ENT_QUOTES);
    $location->latlong = htmlspecialchars(stripslashes($location->latlong), ENT_QUOTES);
    $location->team_id = $location->team_id;
  ...
  function getMatches( $search = false, $limit = false, $order = false, $output = 'OBJECT' )
  {
    ...
    $sql = "SELECT `group`, `round`, `home_team`, `away_team`, DATE_FORMAT(`date`, '%Y-%m-%d %H:%i') AS date, DATE_FORMAT(`date`, '%e') AS day, DATE_FORMAT(`date`, '%c') AS month, DATE_FORMAT(`date`, '%Y') AS year, DATE_FORMAT(`date`, '%H') AS `hour`, DATE_FORMAT(`date`, '%i') AS `minutes`, `match_day`, `location`, `league_id`, `home_points`, `away_points`, `winner_id`, `loser_id`, `post_id`, `season`, `id`, `custom` FROM {$wpdb->leaguemanager_matches}";
  ...
  function getMatch( $match_id )
  {
    ...
    $match = $wpdb->get_results( "SELECT `group`, `round`, `home_team`, `away_team`, DATE_FORMAT(`date`, '%Y-%m-%d %H:%i') AS date, DATE_FORMAT(`date`, '%e') AS day, DATE_FORMAT(`date`, '%c') AS month, DATE_FORMAT(`date`, '%Y') AS year, DATE_FORMAT(`date`, '%H') AS `hour`, DATE_FORMAT(`date`, '%i') AS `minutes`, `match_day`, `location`, `league_id`, `home_points`, `away_points`, `winner_id`, `loser_id`, `post_id`, `season`, `id`, `custom` FROM {$wpdb->leaguemanager_matches} WHERE `id` = {$match_id}" );
    $match = $match[0];
